...
function matchAnswer(
  ...
  for (const answer of answers) {
    ...
    for (const candidate of candidates) {
      ...
      // Exact match
      if (candNorm === norm) return { points: answer.points, matchedAnswer: answer.text };
      // Last-name match for player names (e.g. "James" matches "LeBron James")
      const candWords = candNorm.split(' ');
      const userWords = norm.split(' ');
      for (const uWord of userWords) {
        if (
          uWord.length >= 3 &&
          candWords[candWords.length - 1] === uWord &&
          candWords.length > 1
        ) {
          return { points: answer.points, matchedAnswer: answer.text };
        }
      }
      // Single-word answer substring match (e.g. "defense" in a team nickname)
      if (
        candWords.length === 1 &&
        candWords[0].length >= 4 &&
        (norm.includes(candWords[0]) || candWords[0].includes(norm))
      ) {
        return { points: answer.points, matchedAnswer: answer.text };
      }
    }
...
export default function PowerPlayScreen({ onBack, archiveDate }: Props) {
  ...
  function handleLockIn() {
    ...
    if (currentQ < NUM_QUESTIONS - 1) {
      slideToNextQuestion(currentQ + 1);
    } else {
      stopTimer();
    ...
    return (
      <View style={styles.introCard}>
        ...
        <View style={styles.introRules}>
          ...
          <Text style={styles.introRule}>• You have 90 seconds total</Text>
          <Text style={styles.introRule}>• Match popular answers to score points</Text>
          <Text style={styles.introRule}>• Hit {TARGET_SCORE} points for a perfect score</Text>
        </View>
        <Pressable
  ...
  function renderResults() {
    ...
    const xpDisplay = 500 + totalScore * 5;
      ...
      >
        ...
        {revealedCount === NUM_QUESTIONS && !isArchive && (
          <View style={styles.xpCard}>
            ...
            <Text style={styles.xpCardTotal}>+{xpDisplay}</Text>
